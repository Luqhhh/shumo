...
本项已于 2026-09-11 由三位队员批准：上游先产生每个 `valid_time` 唯一的小时预测节点，重采样层不选择 vintage 或计算权重；以最近已结束十分钟实际 PV 区间均值作边界代理，`RESAMPLE-LIN` 为主方案、PCHIP 为敏感性，输出按十分钟右端点对齐并使用 `kWh = kW / 6`。版本组合、权重和 `epsilon` 仍由 `D_MODEL_Q3` 单独决定。
