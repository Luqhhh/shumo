...
用途：参赛队对 C 线尚未完全锁定的口径逐项表决。历史候选与证据保留在下文。2026-09-12 A在本会话明确批准C-1/C-5（LOAD-A + LF-A）、D_SETTLE与D_MODEL_Q3；当前状态及精确口径以 `configs/decisions.toml` 和 `docs/q3_model.md` 为准。其余候选仍不自动获批。
...
Card C-6 已于2026-09-13由队长确认并落为 `TAIL-EXP2`；随后 C-7 也由队长
单独确认为 `LOAD-HORIZON-A`。两项按要求分别提交，没有批量修改。
...
- **审批状态**：三位队员已确认，`D_RESAMPLE` 批准记录见可解析提交 `9f8f323f0dd28acd7619715faba5319ca80d169a`（`approve: finalize C-4 resampling decision`）；`0b06e37` 是此前的证据提交；版本组合、权重和 `epsilon` 仍归 `D_MODEL_Q3`，不随本项批准。
...
- **审批状态**：队长于2026-09-13单独确认 `LOAD-HORIZON-A`，已补入
  `D_LOAD_FORECAST` 的批准口径；没有把 C-6/C-7 合成一次批量批准。
- **已解决的接口矛盾**：原 `forecast_q3_load` 只允许在00/06/12/18生成最多24小时预测。以06:00版本为例，它只覆盖至次日06:00；但11:50的固定24小时 MPC 需要负载预测至次日11:50，因此同样缺5小时50分钟。这个问题与附件3无关，不能用 `D_PV_TAIL_BASELINE` 顺便处理。
- **批准口径 `LOAD-HORIZON-A`**：仍只在00/06/12/18更新一次 LOAD-A，但每版直接生成未来30小时的不可变负载预测；中间 MPC 只切片未来24小时。这样保留“每6小时因果更新”和“固定24小时窗口”，也不创造新的预测算法。目标时刻所需7/14/21/28日滞后均早于发布时间，公式保持因果。30小时只是覆盖接口，不改变 Q2 的预测更新机制。
...
- **对代码与测试的影响**：每版固定180点、`training_cutoff=issue_time`；中间
  `build_q3_load_window` 只切片144点且不接收 `InfoSet`，从接口上阻止重新预测和
  读取新 actual。到下一发布时间必须换用新版本，旧版本不得继续使用。
