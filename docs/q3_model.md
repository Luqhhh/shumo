...
本文把 C 线已经完成的证据、待确认口径和求解器接口集中到一处，目的是让参赛队
确认后可以直接进入实现。本文件不是批准记录；`configs/decisions.toml` 仍是唯一
机器可读状态来源。在 `D_SETTLE`、`D_LOAD_FORECAST`、`D_MODEL_Q3` 及团队保留的
其他独立 decision 获得人工批准前，不得把本草案实现成正式 runner。
