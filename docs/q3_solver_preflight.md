...
负载预测按队长确认的 C-7 `LOAD-HORIZON-A` 实现：每个00/06/12/18版本一次性
生成30小时并保存不可变 snapshot；中间每10分钟只切出未来24小时，不重新运行
LOAD-A。窗口构造接口不接收 actual/InfoSet，从结构上避免读取本版发布后的真实
负载；到下一发布时间必须使用新版本。
...
跨到次日的 `lookahead_only` 合同量在窗口内按次日对应固定价计入
`lookahead_surrogate_cost`，否则未来外网电会在优化中变成免费能源；该项只用于
当前窗口权衡，不提交、不写 settlement ledger，也不进入正式累计费用。窗口解将
它与新增计划费、调整费、预测紧急费和终端残值分项保存，禁止混入正式结算。
...
`problem/q3_solver.py` 已实现单窗口稀疏 MILP 和独立 validator，但尚未接入正式
年度 runner。当前只允许用小型/合成窗口验证；不得把重叠窗口目标相加冒充实际
总成本，也不得在 runner 完成前返回正式成功。
