...
日期：2026-09-12。状态：**approved，保留v2历史规范**。
当前实现为v2叠加用户批准的[v3年末储能预留](q4_v3_terminal_reserve.md)，新四轨迹各48096步完整核验及两主方案模板读回通过。
批准人：本会话用户；批准日期：2026-09-12；来源见[批准记录](approvals/2026-09-12-q4.md)。
...
两张模型卡已按本会话授权批准，预测/控制/窗口/年度范围收进各自完整模型decision。
模板与比较分别由已批准的D_TIME_TEMPLATE_EXPORT_Q4、D_EVAL_Q4控制；
精确scope_cases及确认字段由gate检查。实施证据见[q4_implementation_status.md](q4_implementation_status.md)，未修改selected_runs。
...
v1随规模增大的相对容差放宽已有contract。费用用未舍入值独立重算，验收总差
...
1. 已完成：落实两张模型卡及Q4范围的预测/反馈/窗口/年度/模板/比较决定；保留真实批准记录。
   正式gate需逐项检查范围和确认字段，不能仅用Q1导出approved放行Q4。
2. 已完成：共享合同ledger与反馈层、冻结/版本/负荷优先/35.5元核验，独立验证器先于全年。
3. 已完成：因果预测与真实历史预检、完整24小时及年末必要覆盖核验。
4. 已完成：共用MILP和两case编排、完美预测测试、实际单日/跨日/一周核验。
5. 已完成：v3来源/checkpoint绑定，四条独立全年重跑、实际预留/端点及两主方案导出读回通过，形成候选。
6. 已完成：两价格对照完整重跑、6h/24h及月指标、八展示日图和论文结果表；
   方法与预留已补全，草稿编译通过，费用改善、紧急量增加及PV长窗负结果如实保留。

设计交付轮仅完成设计和只读证据脚本；本次补齐批准记录及范围门槛。
后续实施已通过工程与短期真实回放校验。全年轨迹、真实Excel数值读回和AI人工核验
以实施状态中的实际证据为准，不因规范批准或合成验证改写为通过。
