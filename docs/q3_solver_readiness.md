...
上述能力均已有合成 contract tests。真实1日、7日和完整2月审计均已通过；全年
诊断运行正在执行。真实数据验证失败必须保留失败证据，不使用 `xfail` 掩盖。
...
1. 真实1日、7日和完整2月审计已通过
2. 完成并审计正在运行的全年诊断
3. 全年稳定且经团队复核后才进入正式selected run与Excel导出
```
...
通过。全年诊断仍在执行；在全年验证完成并经团队复核前，不得选择为正式run。
