...
- 2026-09-12 A明确批准 Q3 的 `D_LOAD_FORECAST`（LOAD-A + LF-A）、`D_SETTLE` 和 `D_MODEL_Q3`，完整口径见 `docs/q3_model.md`。Q3因果预测、滚动MILP、实际回放、跨日SOC和内部artifacts已实现；本地附件/哈希不完整时正式入口会留失败证据，不能生成假结果。Q2/Q4模型与D_EVAL仍待各自批准。
