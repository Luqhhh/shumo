...
将题目要求的两类条件分别记为问题四（2）和问题四（3）。前者每日0:00提交144个区间的初始合同$G_k^0$，当日不再调整；后者增加已发布光伏预报，并在6、12、18时调整当日尚未开始区间的合同。跨日窗口内的次日购电量只用于预期调度，次日0:00才形成实际合同。
...
\IfFileExists{tables/q4_results_paper.tex}{\input{tables/q4_results_paper}}{
  \IfFileExists{tables/q4_results.tex}{\input{tables/q4_results}}{回放结果待补充。}
}
\FloatBarrier
