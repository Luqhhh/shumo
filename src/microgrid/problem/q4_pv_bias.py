"""Delegated long-lead PV bias correction, learning only saved, realized forecast pairs."""
...
from .q4_common import ACTION_START, STEP, Q4Error, nonnegative, pv_bias_parameters
...
class LongPVBias:
    def __init__(self):
        ...
        self.issue_count = 0
        self.chain = hashlib.sha256(b"PV-BIAS-LONG empty at 2025-02-01").digest()
        ...
        if pairs:
            ...
            for issue, bucket, group, original, historical in pairs:
                ...
                self.realized.append(
                    (target, bucket, group, actual_kw - original, abs(actual_kw - historical))
                )
    ...
    def apply(self, issue: dt.datetime, original, historical):
        ...
        if (
            ...
            raise Q4Error("invalid_forecast", "invalid PV bias initialization or dimensions")
        ...
        for index, (f_kw, h_kw) in enumerate(zip(original, historical, strict=True), 1):
            ...
            if bucket is not None and not cold and n:
                alpha = 0.0
            bias = 0.0 if alpha == 1.0 else m_f
            value = f_kw if bias == 0.0 else max(0.0, f_kw + bias)
            values.append(value)
            points.append(
                {
                    ...
                    "history_kw": h_kw,
                    "sample_count": n,
                    "original_signed_error_sum_kw": f_sum,
                    "history_abs_error_sum_kw": h_sum,
                    "original_bias_kw": m_f,
                    "history_mae_kw": m_h,
                    "bias_correction_kw": bias,
                    "prediction_kw": value,
                }
            )
