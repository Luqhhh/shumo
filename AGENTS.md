...
3. **未经批准的 decision 不得实现**：`configs/decisions.toml` 中 `pending` / `proposed` 的事项都不能实现；只有 `approved` 且 `confirmed_by`/`confirmed_at` 完整时才放行。Agent 只可整理证据。模型 gate 按 case 拆分，不得用一个全局 D_MODEL 代替五个 case 的批准范围。
...
11. **统一模型结果与导出边界**：runner 只产生 `CaseResult` / `IntervalResult`；不得直接写 Excel。正式数值导出必须等待 `D_TIME_TEMPLATE_EXPORT` approved。
12. **不得绕过 InfoSet 因果隔离**：计划器输入使用 `InfoSet.from_raw`，不得把未来 actual 直接传入；未来 actual 只用于回放结算。
13. **不得绕开正式阻断**：`prepare_submission.py --mode final` 和 `microgrid run --case ...` 必须按 pending decisions 与正式 run artifacts 的真实状态放行或阻断。
14. **保护用户已存在文件**：执行前读取 README、AGENTS、配置和 `git status`；不 reset --hard、不强制推送、不删除原始数据。
